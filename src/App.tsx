...
import { ProductImage, GeneratedImage, ShotType } from "./types";
import {
...
export default function App() {
  ...
  const handleStartApp = () => {
    ...
    if (introAudioUrl) {
      ...
      audio
        .play()
        .catch((e) =>
          console.log("Audio play failed (interaction policy):", e),
        );
    }
    setAppState("welcome");
  };
  ...
  const detectProductName = async (file: File) => {
    try {
      ...
      const prompt =
        "Identifikasi produk dalam gambar ini. Berikan nama yang singkat, detail, dan deskriptif dalam Bahasa Indonesia (maksimal 4 kata, contoh: 'Kemeja Flanel Coklat', 'Botol Serum Vitamin C'). Jawab HANYA dengan nama produk.";
      ...
      const result = await response.json();
      return (
        result.candidates?.[0]?.content?.parts?.[0]?.text.trim() || file.name
      );
    } catch (error) {
  ...
  const removeProductImage = (indexToRemove: number) => {
    setProductImages((prev) => {
      ...
      if (appState === "selecting") {
        if (
          newImages.length === 0 ||
          (generationMode === "model" && !modelImage)
        ) {
          handleReset();
        } else {
  ...
  const handleGetRecommendations = async (isReload = false) => {
    ...
    setIsLoading(true);
    setLoadingMessage(
      isReload ? "Memuat ulang rekomendasi..." : "Mendeteksi produk...",
    );
    setApiError("");
    ...
    try {
      ...
      let imageParts = await Promise.all(
        productImages.map((p) => fileToGenerativePart(p.file)),
      );
      ...
      const finalBackgrounds = [
        "Backdrop Putih Polos",
        ...(parsedJson.backgrounds || []),
      ];
      ...
      setApiError(
        "Gagal mendapatkan rekomendasi. Coba lagi dengan gambar lain.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGenerateCommercialScript = async () => {
    ...
    try {
      ...
      if (script) {
        ...
        setCommercialScript(
          `Tampil memukau dengan gaya yang sempurna. Dapatkan koleksi ${namesToUse} eksklusif ini sekarang juga.`,
        );
      }
    } catch (error) {
      console.error("Script generation failed:", error);
      setCommercialScript(
        "Temukan gaya terbaikmu hari ini dengan koleksi terbaru kami. Kualitas premium untuk penampilan yang tak terlupakan.",
      );
    } finally {
  ...
  const handleGlobalGenerateAudio = async () => {
    ...
    const tonePrefix =
      TONE_OPTIONS.find((t) => t.value === selectedTone)?.prompt || "";
  ...
  const generateDynamicShots = async (
    imageParts: any[],
    mode = "model",
    style = "COMMERCIAL",
    count = 10,
    ...
    try {
      ...
      const payload = {
        contents: [
          {
            parts: [
              { text: `Generate ${count} sequential storyboard shots.` },
              ...imageParts,
            ],
          },
        ],
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (generationMode === "model" && modelImage) {
        ...
        currentShotTypes = await generateDynamicShots(
          productImageParts,
          "model",
          storyboardType,
          sceneCount,
      ...
      for (let i = 0; i < shotsToGenerate.length; i++) {
        ...
        setLoadingMessage(
          `Menciptakan Scene... (${i + 1}/${shotsToGenerate.length}: ${shotName})`,
        );
        ...
        if (base64Data) {
          const processedUrl = await adjustImageAspectRatio(
            base64Data,
            aspectRatio,
          );
          const videoRatioText =
            aspectRatio === "9:16"
              ? "--ar 9:16"
              : aspectRatio === "16:9"
                ? "--ar 16:9"
          ...
          if (generationMode === "product")
            videoPromptText += ", NO human faces";
      ...
      setApiError(
        error.message || "Gagal menghasilkan gambar. Silakan coba lagi.",
      );
      setAppState("result");
    } finally {
      setIsLoading(false);
  ...
  const handleRegenerate = async (imageId: number, revisionText: string) => {
    ...
    try {
      ...
      if (!revisionText || revisionText.trim() === "") {
        ...
        const imageToRevisePart = await dataUrlToGenerativePart(
          originalImage.url,
        );
  ...
  const isReadyForSelection = () => {
    ...
    if (generationMode === "model")
      return modelImage !== null && productImages.length > 0 && !isLoading;
    if (generationMode === "product")
      return productImages.length > 0 && !isLoading;
    return false;
  };
    ...
    return (
      <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center font-sans">
        <div className="bg-[#FFE600] border-4 border-black p-8 neo-shadow text-center">
          <h2 className="text-2xl font-black uppercase tracking-wider mb-4 text-black animate-pulse">
            Memuat...
          </h2>
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] font-sans text-black antialiased flex flex-col selection:bg-[#00E5FF] selection:text-black">
      {appState !== "intro" && (
        <header className="px-6 py-4 border-b-[4px] border-black bg-white flex justify-between items-center top-0 z-20 sticky neo-shadow-sm">
          <div className="flex items-center gap-3">
            <div className="bg-[#FFDE59] border-2 border-black w-10 h-10 flex items-center justify-center font-black text-xl neo-shadow-sm transform -rotate-6">
              M
            </div>
            <div>
              <h1 className="text-xl md:text-2xl font-black tracking-tighter text-black uppercase leading-none">
                Magic UGC Generator
              </h1>
              <p className="text-[10px] font-bold text-black border-t-2 border-black mt-1 pt-0.5 uppercase tracking-widest inline-block bg-[#00E5FF] px-1">
                by growwithdedy
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {userData?.role === "admin" && (
              <button
                ...
                className="neo-btn bg-[#00E5FF] text-black font-black uppercase px-4 py-2 border-2 border-black text-sm"
              >
                ADMIN
              </button>
            )}
            <button
              onClick={logout}
              className="neo-btn bg-black text-white font-black uppercase px-3 py-2 border-2 border-black hover:bg-gray-800"
              title="Logout"
            >
              <svg
            ...
            {appState === "result" && (
              <button
                ...
                className="neo-btn bg-white text-black font-black uppercase px-4 py-2 border-2 border-black text-sm"
              >
                KEMBALI
              </button>
            )}
            {appState === "selecting" && generatedImages.length > 0 && (
              <button
                onClick={handleViewResults}
                className="neo-btn bg-[#FF90E8] text-black font-black uppercase px-4 py-2 border-2 border-black text-sm"
              >
                RIWAYAT HASIL
              </button>
            )}
            <button
              onClick={handleReset}
              className={`neo-btn font-black uppercase px-4 py-2 text-sm border-2 border-black ${appState === "welcome" ? "bg-gray-200 text-gray-500 cursor-not-allowed shadow-none hover:transform-none" : "bg-[#FF5252] text-white"}`}
              disabled={appState === "welcome"}
            >
              RESET
            </button>
          </div>
      ...
      <main
        className={`${appState !== "intro" ? "px-4 py-6 md:px-8 md:py-8" : "p-0"} flex-grow`}
      >
          ...
          {appState === "intro" && (
            <div className="flex flex-col items-center justify-center h-screen w-full bg-[#FFE600] text-black animate-scale-in border-[12px] border-black m-0 p-0 box-border">
              <div className="text-center space-y-8 p-10 max-w-4xl">
                ...
                <h1
                  className="text-7xl md:text-9xl font-black tracking-tighter mb-2 uppercase"
                  style={{ textShadow: "6px 6px 0px #fff, 8px 8px 0px #000" }}
                >
                  MAGIC
                  <br />
                  UGC
                ...
                <div className="mt-16">
                  <button
                    ...
                    className="neo-btn bg-[#00E5FF] text-black text-2xl font-black uppercase px-16 py-6 border-4 border-black neo-shadow"
                  >
                    Mulai Sekarang!
                  </button>
                </div>
              ...
              {generationMode === null ? (
                <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                  <div
                    ...
                    className="neo-card bg-[#A3E635] p-10 cursor-pointer hover:bg-[#86efac] transition-colors flex flex-col items-center text-center group"
                  >
                    ...
                    <h3 className="text-2xl font-black text-black mb-3 uppercase border-b-4 border-black pb-2">
                      Model Generator
                    </h3>
                  </div>
                  <div
                    onClick={() => {
                      playClickSound();
                      setGenerationMode("product");
                    }}
                    className="neo-card bg-[#00E5FF] p-10 cursor-pointer hover:bg-[#67e8f9] transition-colors flex flex-col items-center text-center group"
                  >
                    <div className="w-24 h-24 mb-6 bg-white border-4 border-black text-black flex items-center justify-center neo-shadow-sm group-hover:-translate-y-2 transition-transform">
                      <svg
                        className="w-12 h-12"
                ...
                <div className="mt-8">
                  {generationMode === "model" ? (
                    <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                      <UploadArea
                        ...
                        files={
                          modelImage
                            ? [{ file: modelImage, name: modelImage.name }]
                            : []
                        }
                        title="1. UPLOAD MODEL"
                        description="Maks. 1 Foto"
                        onRemoveImage={removeModelImage}
                      />
                      <UploadArea
                        onDrop={onProductDrop}
                        files={productImages}
                        title="2. UPLOAD PRODUK"
                        description="Maks. 2 Foto"
                        maxFiles={2}
                  ...
                  {productImages.length > 0 && (
                    <div className="max-w-4xl mx-auto mt-10 neo-card bg-white p-8 text-left relative">
                      <div className="absolute -top-4 -left-4 bg-[#FF90E8] border-4 border-black px-4 py-1 font-black uppercase text-xl neo-shadow-sm transform -rotate-3">
                        DETAIL PRODUK
                      </div>
                      <div className="space-y-6 mt-4">
                        {productImages.map((product, index) => (
                          <div
                            key={product.id}
                            className="flex flex-col md:flex-row gap-4 items-center md:items-start p-4 bg-gray-100 border-4 border-black neo-shadow-sm"
                          >
                            <div className="w-20 h-20 shrink-0 bg-white border-2 border-black p-1">
                              <img
                            ...
                            <div className="flex-grow w-full">
                              <label className="text-xs font-black text-black uppercase tracking-widest block mb-2 bg-[#A3E635] inline-block px-2 py-1 border-2 border-black">
                                ITEM {index + 1}
                              </label>
                              <input
                                type="text"
                                ...
                                className="w-full p-3 border-4 border-black bg-white text-black font-bold focus:bg-[#FFE066] neo-shadow-sm"
                                disabled={product.name === "Mendeteksi..."}
                              />
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="mt-8 flex justify-end border-t-4 border-black pt-6">
                        {!isDetailsConfirmed ? (
                          <button
                            ...
                            className="neo-btn bg-[#00E676] text-black font-black uppercase py-3 px-8 border-[4px] border-black"
                          >
                            KONFIRMASI DETAIL
                          </button>
                        ) : (
                          <div className="flex items-center gap-4">
                            <div className="bg-[#A3E635] px-4 py-2 border-4 border-black neo-shadow-sm font-black uppercase">
                              ✅ DATA TERKUNCI
                            </div>
                            <button
                              onClick={() => setIsDetailsConfirmed(false)}
                              className="text-sm font-black border-b-2 border-black uppercase"
                            >
                              EDIT ULANG
                            </button>
                          </div>
                  ...
                  {apiError && (
                    <p className="text-white bg-[#FF5252] border-4 border-black font-black text-lg mt-6 p-4 neo-shadow inline-block uppercase">
                      {apiError}
                    </p>
                  )}
                  <div className="mt-12 flex flex-col items-center justify-center gap-4">
                    ...
                    <button
                      onClick={handleReset}
                      className="text-sm text-black font-black uppercase border-b-2 border-black hover:bg-[#FF5252] hover:text-white px-2 mt-4"
                    >
                      X BATAL & GANTI MODE
                    </button>
                  </div>
            ...
            <div className="animate-fade-in-up mt-6">
              <div className="flex flex-col md:flex-row items-center justify-between mb-10 bg-white border-4 border-black p-6 neo-shadow">
                <div>
                  ...
                  <p className="text-black font-bold text-sm bg-[#FFDE59] inline-block px-2 border-2 border-black mt-2 uppercase">
                    ATUR KOMPOSISI VISUAL ANDA
                  </p>
                </div>
                <div className="text-lg font-black text-white bg-black border-4 border-black px-4 py-2 mt-4 md:mt-0 neo-shadow-sm uppercase transform rotate-2">
                  STEP 2 / 3
                </div>
              </div>
              <div className="grid lg:grid-cols-12 gap-8 lg:gap-12">
                <div className="lg:col-span-4 lg:sticky lg:top-28 z-10 self-start">
                  <div className="neo-card bg-[#FF90E8] p-6 relative mt-4">
                    <div className="absolute -top-5 left-4 bg-white border-4 border-black px-3 py-1 font-black uppercase neo-shadow-sm">
                      ASET AKTIF
                    </div>
                    <div className="flex flex-wrap gap-4 mt-4">
                      ...
                      {productImages.map((p, i) => (
                        <AssetThumbnail
                          ...
                          onReplace={(file) => handleReplaceProduct(i, file)}
                        />
                      ))}
                ...
                <div className="lg:col-span-8 space-y-12">
                  <section className="neo-card bg-white p-6">
                    <h3 className="text-xl font-black uppercase tracking-tight mb-6 bg-[#A3E635] inline-block px-3 border-2 border-black">
                      1. STYLE DIRECTION
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {STORYBOARD_TYPES.map((type) => (
                        <div
                          ...
                          onClick={() => {
                            playClickSound();
                            setStoryboardType(type.value);
                          }}
                          className={`cursor-pointer p-4 border-[4px] border-black transition-all flex flex-col items-start gap-4 neo-shadow-sm ${storyboardType === type.value ? "bg-[#FFDE59] transform -translate-y-1" : "bg-gray-50 hover:bg-[#FDF8E1]"}`}
                        >
                          <div className="text-black bg-white border-2 border-black p-2 neo-shadow-sm">
                            {type.icon}
                          </div>
                          <div>
                  ...
                  <section className="neo-card bg-white p-6">
                    <h3 className="text-xl font-black uppercase tracking-tight mb-6 bg-[#00E5FF] inline-block px-3 border-2 border-black">
                      2. FOKUS KOMPOSISI
                    </h3>
                    <div className="flex flex-wrap gap-4">
                      {(generationMode === "model"
                        ...
                        <button
                          ...
                          className={`flex-1 min-w-[140px] py-4 px-4 font-black text-sm uppercase transition-all border-4 border-black ${focusMode === mode.value ? "bg-black text-white neo-shadow" : "bg-white text-black hover:bg-gray-200 neo-shadow-sm hover:-translate-y-1"}`}
                        >
                          {mode.label}
                        </button>
                      ))}
                  ...
                  <section className="grid md:grid-cols-2 gap-8">
                    <div className="neo-card bg-[#F4F4F0] p-6">
                      <label className="text-lg font-black uppercase mb-4 block">
                        JUMLAH SCENE
                      </label>
                      <div className="flex flex-col gap-3">
                        {SCENE_COUNT_OPTIONS.map((opt) => (
                          <button
                            ...
                            onClick={() => {
                              playClickSound();
                              setSceneCount(opt.value);
                            }}
                            className={`w-full py-3 px-4 text-sm font-black uppercase transition-all border-[3px] border-black text-left ${sceneCount === opt.value ? "bg-[#FF5252] text-white neo-shadow-sm transform translate-x-1" : "bg-white text-black hover:bg-gray-100"}`}
                          >
                            {opt.label}
                          </button>
                        ))}
                    ...
                    <div className="neo-card bg-[#F4F4F0] p-6">
                      ...
                      <select
                        ...
                        className="w-full bg-white border-[4px] border-black text-black text-sm font-black py-4 px-4 focus:outline-none focus:bg-[#FFE066] neo-shadow-sm cursor-pointer mb-4"
                      >
                        {backgroundRecommendations.map((bg, i) => (
                      ...
                      {selectedBackground === "custom_bg" && (
                        <input
                          type="text"
                          placeholder="KETIK DESKRIPSI LATAR..."
                          value={manualBackground}
                          onChange={(e) => setManualBackground(e.target.value)}
                          className="w-full bg-white border-[4px] border-black text-black placeholder-gray-400 py-4 px-4 text-sm font-black focus:outline-none focus:bg-[#FFE066] neo-shadow-sm animate-fade-in-up"
                        />
                      )}
                    </div>
                  </section>
                  <div className="pt-8 pb-12 flex flex-col items-center">
                    <button
                      ...
                      disabled={isLoading}
                      className="neo-btn bg-black text-[#FFDE59] text-2xl font-black py-5 px-16 border-4 border-black hover:bg-gray-900 transition-all uppercase inline-block"
                    >
                      GENERATE SEKARANG!
                    </button>
                  </div>
          ...
          {(appState === "generating" || appState === "result") && (
            <div className="mt-8">
              <div className="inline-block bg-[#00E5FF] border-4 border-black px-8 py-3 transform -rotate-1 neo-shadow mb-12 mx-auto flex items-center justify-center max-w-2xl">
                <h2 className="text-2xl md:text-4xl font-black text-black uppercase tracking-tight text-center">
                  {isLoading
                    ? "MERANCANG STORYBOARD..."
                    : `HASIL: ${generatedImages.length} SCENES`}
                </h2>
              </div>
              {isLoading && (
                <div className="text-center p-12 neo-card bg-white max-w-md mx-auto">
                  ...
                  <p className="text-xl font-black text-black uppercase bg-[#FFDE59] inline-block px-4 py-2 border-4 border-black neo-shadow-sm">
                    {loadingMessage}
                  </p>
                </div>
              )}
              {apiError && !isLoading && (
                <p className="text-black bg-[#FF5252] border-4 border-black font-black text-xl p-6 text-center neo-shadow mx-auto max-w-2xl uppercase">
                  {apiError}
                </p>
              )}
              ...
              {appState === "result" && (
                <div className="mt-24 max-w-6xl mx-auto">
                  <div className="mb-12 border-b-8 border-black pb-6 flex items-end justify-between">
                    <div>
                      <h2 className="text-5xl font-black text-black uppercase tracking-tight">
                        AUDIO STUDIO
                      </h2>
                      <p className="text-xl text-black font-bold bg-[#A3E635] inline-block px-3 mt-2 border-2 border-black uppercase">
                        BUAT VOICEOVER PROFESIONAL
                      </p>
                    </div>
                  </div>
                  <div className="neo-card bg-white overflow-hidden text-black flex flex-col xl:flex-row">
                    <div className="xl:w-1/3 p-8 border-b-4 xl:border-b-0 xl:border-r-4 border-black bg-[#FDF8E1]">
                      <div className="flex items-center gap-3 mb-8 bg-black text-white p-3 neo-shadow-sm inline-flex">
                      ...
                      <div className="space-y-6">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-white border-4 border-black p-4 text-black font-bold focus:outline-none focus:bg-[#FFE066] neo-shadow-sm"
                          >
                        ...
                        <button
                          onClick={handleGenerateCommercialScript}
                          disabled={isGeneratingScript}
                          className="neo-btn w-full py-4 mt-4 font-black bg-[#FF90E8] text-black border-4 border-black uppercase"
                        >
                          {isGeneratingScript
                            ? "MENULIS..."
                            : "AUTO-WRITE SCRIPT"}
                    ...
                    <div className="xl:w-1/3 p-8 border-b-4 xl:border-b-0 xl:border-r-4 border-black bg-white">
                      ...
                      <textarea
                        className="w-full h-80 p-6 border-[4px] border-black bg-[#F4F4F0] text-black font-bold text-lg resize-none focus:outline-none focus:bg-[#FFE066] neo-shadow-sm"
                    ...
                    <div className="xl:w-1/3 p-8 bg-[#E2F0CB] flex flex-col justify-between">
                      <div>
                        <div className="flex items-center gap-3 mb-8 bg-black text-[#A3E635] p-3 neo-shadow-sm inline-flex">
                          <h3 className="font-black text-xl uppercase">
                            AUDIO PREVIEW
                          </h3>
                        </div>
                        {audioUrl ? (
                          <div className="bg-white border-4 border-black p-6 text-center neo-shadow-sm animate-fade-in-up">
                            <div className="w-16 h-16 bg-[#A3E635] border-4 border-black rounded-full flex items-center justify-center mx-auto mb-4 neo-shadow-sm">
                              <svg
                            ...
                            <audio
                              controls
                              src={audioUrl}
                              className="w-full h-12 mb-6 border-2 border-black bg-gray-100"
                            />
                            <a
                              href={audioUrl}
                              download="magic-affiliate-gen.wav"
                              className="neo-btn block w-full bg-black text-[#FFDE59] text-sm font-black uppercase py-4 border-4 border-black text-center"
                            >
                              DOWNLOAD WAV
                            </a>
                          </div>
                        ...
                        {audioError && (
                          <p className="text-white bg-[#FF5252] font-black mt-6 p-4 border-4 border-black neo-shadow-sm uppercase">
                            {audioError}
                          </p>
                        )}
                      </div>
                      <button
                        onClick={handleGlobalGenerateAudio}
                        disabled={isGeneratingAudio || !commercialScript.trim()}
                        className={`neo-btn w-full mt-8 py-5 text-xl font-black uppercase border-4 border-black ${isGeneratingAudio ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-[#FFDE59] text-black"}`}
                      >
                        {isGeneratingAudio ? "MEMPROSES..." : "GENERATE AUDIO"}
                      </button>
                    </div>
                  </div>
                  <div className="mt-24 mb-24 text-center">
                    <div className="inline-block bg-[#00E5FF] border-4 border-black px-6 py-2 mb-8 transform rotate-1 neo-shadow-sm">
                    ...
                    {!showVideoOptions ? (
                      <button
                        ...
                        className="neo-btn bg-black text-white px-12 py-5 font-black uppercase border-4 border-black text-xl inline-flex items-center gap-4"
                      >
                      ...
                      <div className="flex flex-col md:flex-row justify-center gap-6 animate-fade-in-up">
                        <button
                          ...
                          className="neo-btn bg-[#FF90E8] text-black px-8 py-5 font-black uppercase border-4 border-black inline-flex items-center justify-center gap-3 w-full md:w-auto text-lg"
                        >
                          META AI
                        </button>
                        <button
                          ...
                          className="neo-btn bg-white text-black px-8 py-5 font-black uppercase border-4 border-black inline-flex items-center justify-center gap-3 w-full md:w-auto text-lg"
                        >
                          GROK AI
                        </button>
                        <button
                          ...
                          className="neo-btn bg-[#A3E635] text-black px-8 py-5 font-black uppercase border-4 border-black inline-flex items-center justify-center gap-3 w-full md:w-auto text-lg"
                        >
                          GOOGLE FLOW
                        </button>
                      </div>
            ...
            <div className="fixed inset-0 bg-[#FDFBF7]/90 z-50 flex items-center justify-center animate-fade-in-up p-4">
              <div className="bg-white border-[6px] border-black p-8 max-w-md w-full neo-shadow relative">
                <button
                  ...
                  className="absolute -top-6 -right-6 w-12 h-12 bg-[#FF5252] border-4 border-black text-black font-black text-2xl neo-shadow-sm hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all"
                >
                  X
                </button>
                <div className="text-center">
                  <h3 className="text-3xl font-black uppercase mb-2 bg-[#FFDE59] inline-block px-4 py-1 border-2 border-black transform -rotate-2">
                    FORMAT OUTPUT
                  </h3>
                  <p className="text-black font-bold mb-8 mt-4">
                    Pilih rasio untuk {sceneCount} variasi otomatis Anda.
                  </p>
                  <div className="mb-10">
                    <div className="grid grid-cols-3 gap-4">
                      {ASPECT_RATIOS.map((ratio) => (
                        <button
                          ...
                          className={`py-6 px-2 text-xl font-black uppercase transition-all border-[4px] border-black ${aspectRatio === ratio ? "bg-[#00E5FF] text-black neo-shadow transform -translate-y-1" : "bg-white text-black hover:bg-gray-100"}`}
                        >
                          {ratio}
                        </button>
                      ))}
                  ...
                  <CustomButton
                    ...
                    disabled={isLoading}
                    className="w-full py-5 text-2xl bg-[#A3E635]"
                  >
                    🚀 GENERATE!
                  </CustomButton>
                </div>
      ...
      <footer className="p-8 border-t-[4px] border-black text-center bg-[#FFDE59] mt-auto flex flex-col items-center justify-center gap-6">
        <div className="flex flex-col md:flex-row items-center justify-center gap-2">
          <p className="text-sm text-black font-black uppercase tracking-widest">
            2026 Magic UGC Generator
          </p>
          <p className="text-sm text-black font-bold border-l-0 md:border-l-4 border-black pl-0 md:pl-2">
            Licensed by{" "}
            <span className="font-black bg-white px-2 border-2 border-black inline-block transform -rotate-2">
              growwithdedy
            </span>
          </p>
        </div>
