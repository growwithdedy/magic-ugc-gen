...
import { Instagram, ExternalLink, Key, X, Lightbulb, Eye, EyeOff, Save, Trash2, Lock } from 'lucide-react';
import {
  VOICE_OPTIONS,
  LANGUAGE_OPTIONS,
  TONE_OPTIONS,
...
export default function App() {
  ...
  useEffect(() => {
    ...
    } else if (userData?.status === 'approved' && !userData.geminiApiKey) {
      // Auto show modal if user is approved but has no API key
      setShowApiKeyModal(true);
    }
  }, [userData]);

  const saveGeminiApiKey = async (newKey: string) => {
    if (!user) return;
  ...
  const getActiveApiKey = () => geminiApiKey || '';
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center">
        <div className="animate-spin rounded-full h-16 w-16 border-t-8 border-b-8 border-black"></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] font-sans text-black antialiased flex flex-col selection:bg-[#00E5FF] selection:text-black">
      ...
      {/* API KEY MODAL - CLEAN & BEAUTIFIED */}
      {showApiKeyModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in text-black">
          <div className="bg-white border-4 border-black max-w-lg w-full neo-shadow relative rounded-[32px] overflow-hidden">
            ...
            {/* New Colored Header with Divider */}
            <div className="bg-[#FFDE59] border-b-4 border-black p-8 flex flex-col items-center gap-2 relative">
              ...
              <div className="bg-white p-4 border-4 border-black neo-shadow-sm rounded-2xl mb-2">
                <Key size={32} />
              </div>
            ...
            <div className="p-10 space-y-8 text-center bg-white">
              <div className="space-y-4">
                ...
                <a
                  ...
                  className="neo-btn w-full bg-[#00E5FF] border-4 border-black px-6 py-4 text-sm font-black uppercase flex items-center justify-center gap-3 hover:scale-[1.02] transition-all neo-shadow-sm animate-pulse-glow rounded-3xl"
                >
              ...
              <div className="space-y-4 pt-2 border-t-2 border-dashed border-gray-200">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Masukkan Gemini API Key Anda Di Sini..."
                    className="w-full p-5 border-4 border-black bg-[#FDFBF7] font-black text-lg focus:bg-white outline-none transition-colors pr-14 rounded-3xl text-center"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-6 top-1/2 -translate-y-1/2 text-black hover:scale-110 transition-transform"
                  >
              ...
              <div className="flex flex-col gap-4">
                <button
                  onClick={async () => { await saveGeminiApiKey(geminiApiKey); setShowApiKeyModal(false); }}
                  disabled={isSavingApiKey}
                  className="neo-btn bg-[#00E5FF] text-black border-4 border-black py-5 font-black uppercase text-xl neo-shadow rounded-3xl hover:-translate-y-1 transition-transform"
                >
                  {isSavingApiKey ? 'MENYIMPAN...' : 'SIMPAN & AKTIFKAN AKUN'}
                </button>

                <button
                  onClick={() => { setGeminiApiKey(''); saveGeminiApiKey(''); }}
                  className="text-xs font-black text-gray-400 uppercase hover:text-[#FF5252] transition-colors"
                >
                  Hapus / Reset API Key
                </button>
              </div>

              <div className="flex items-center justify-center gap-2 text-[10px] font-black text-gray-300 uppercase tracking-tighter pt-4">
      ...
      {appState !== 'intro' && (
        <header className="px-6 py-4 border-b-[4px] border-black bg-white flex justify-between items-center top-0 z-20 sticky neo-shadow-sm">
          ...
          <div className="flex items-center gap-2 md:gap-3 flex-wrap justify-end">
            <button onClick={handleReset} className={`neo-btn font-black uppercase px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm border-2 border-black flex-shrink-0 rounded-lg ${appState === 'welcome' ? "bg-gray-200 text-gray-500 cursor-not-allowed shadow-none hover:transform-none" : "bg-white text-black"}`} disabled={appState === 'welcome'}>RESET</button>
            <button
              onClick={() => setShowApiKeyModal(true)}
              className={`neo-btn bg-[#FFDE59] border-2 border-black w-10 h-10 md:w-11 md:h-11 flex items-center justify-center transition-all relative rounded-xl ${!geminiApiKey ? 'animate-pulse-glow ring-4 ring-[#FFDE59]/50' : 'hover:scale-110'}`}
              title="API Key Configuration"
            >
              <Key size={20} className="text-black" />
              {!geminiApiKey && (
                <span className="absolute -top-1 -right-1 w-3 h-3 bg-[#FF5252] border-2 border-black rounded-full"></span>
              )}
            </button>
            {userData?.role === 'admin' && (
              <button onClick={() => setShowAdminPanel(true)} className="neo-btn bg-[#00E5FF] flex-shrink-0 text-black font-black uppercase px-3 py-1.5 md:px-4 md:py-2 border-2 border-black text-xs md:text-sm rounded-lg">Admin</button>
            )}
            {appState === 'result' && <button onClick={handleBackToSelection} className="neo-btn bg-white text-black font-black uppercase px-3 py-1.5 md:px-4 md:py-2 border-2 border-black text-xs md:text-sm rounded-lg">KEMBALI</button>}
            {appState === 'selecting' && generatedImages.length > 0 && <button onClick={handleViewResults} className="neo-btn bg-[#FF90E8] text-black font-black uppercase px-3 py-1.5 md:px-4 md:py-2 border-2 border-black text-xs md:text-sm rounded-lg">RIWAYAT HASIL</button>}
            <button onClick={logout} className="neo-btn bg-black text-white flex-shrink-0 font-black uppercase px-2 py-1.5 md:px-3 md:py-2 border-2 border-black text-xs md:text-sm rounded-lg">
